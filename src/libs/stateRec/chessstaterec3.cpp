...
bool ChessStateRec3::start(ggp_robot::GetState::Request &req, ggp_robot::GetState::Response &res) {
  PRINT("[SREC] Starting...");
  ...
  // ========================================================================
  // FINAL STEP: prepare response values
  // ========================================================================

  // average over normal vector
  Eigen::Vector3f normalVectorFinal;
  for(boost::circular_buffer<Eigen::Vector3f>::iterator it = normalVector_buffer.begin();
      it != normalVector_buffer.end();
      ++it) {
    normalVectorFinal += (*it);
  }
  normalVectorFinal *= 1.0/normalVector_buffer.size();
  res.normalVector.x = normalVectorFinal(0);
  res.normalVector.y = normalVectorFinal(1);
