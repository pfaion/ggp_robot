...
class BoardPoint: public cv::Point3f{
  public:
    static double FIELD_SIZE;
    BoardPoint(float x, float y): cv::Point3f(x*FIELD_SIZE, y*FIELD_SIZE, 0.0f) {};
    BoardPoint(float x, float y, float z): cv::Point3f(x*FIELD_SIZE, y*FIELD_SIZE, z*FIELD_SIZE) {};
};
...
class ImageConverter
{
  ...
  void drawBoardMarker(){
    ...
    for(int i=0; i<=2; i++){
      for(int j=0; j<=0; j++){
        BoardPoint p(offset+i, offset+j);
        pubBoardMarker(p, (i%2==0) != (j%2==0), i+4*j);
      }
    }
  }

  void pubBoardMarker(BoardPoint p, bool white, int id) {
